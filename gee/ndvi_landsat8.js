...
Extract the monthly average value and standard deviation of NDVI from Landsat 8 images
...
// (2) Function to add NDVI band to Landsat 8 image
...
var landsatCollectionWithNDVI = ee.ImageCollection('LANDSAT/LC08/C01/T1_SR')
...
var addFeatures = function(n) {
  ...
  return Export.table.toDrive({collection:finalTable,
                              description: 'L8_month_' + (n + monthStart).toString() + '_year_' + year.toString(),
                              fileFormat: 'CSV'
                              });
  };
...
// (6) Loop through target dates: 01-04-2013 to 31-12-2019
for (var year = 2013; year <= 2019; year += 1) {
  // Define sequence of month to be included
  if (year == 2013) var monthStart = 4;
  else var monthStart = 1;
  var monthEnd = 12;
