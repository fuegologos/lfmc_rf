...
# Remove NA from NDVIcv
db0 <- db0[complete.cases(db0$NDVIcv), ]
...
for (spf_i in seq_along(sp_filt)) {
    ...
    for (iter_i in seq_len(niter)) {
        ...
        out_idx <- CreateSpacetimeFolds(db0,
                                        spacevar = "SITE",
                                        k = k,
                                        seed = seeds[iter_i])
        ...
        for (outfold in seq_along(out_idx$index)) {
            ...
            train_df <- db0[out_idx$index[[outfold]], ]
            test_df <- db0[out_idx$indexOut[[outfold]], ]
            
            # Apply NDVIcv filter
            train_df <- train_df[train_df$NDVIcv < sp_filt[spf_i], ]
