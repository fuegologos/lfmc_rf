...
Extract the monthly average value and standard deviation of NDVI from Landsat 7 images
...
// (2) Function to add NDVI band to Landsat 7 image
...
var landsatCollectionWithNDVI = ee.ImageCollection('LANDSAT/LE07/C01/T1_SR')
...
var addFeatures = function(n) {
  ...
  return Export.table.toDrive({collection:finalTable,
                              description: 'L7_month_' + (n + monthStart).toString() + '_year_' + year.toString(),
                              fileFormat: 'CSV'
                              });
  };
...
// (6) Loop through target dates: 01-02-2000 to 01-04-2013
for (var year = 2000; year <= 2013; year += 1) {
